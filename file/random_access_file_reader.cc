...

  {
    ...
    if (use_direct_io() && is_aligned == false) {
      ...
      *result = Slice(scratch, res_len);
      ...
      LocalOp read_file_loop_timer;
      read_file_loop_timer.begin();
      while (pos < n) {
        read_file_loop_timer.loop_begin();
        size_t allowed;
        if (rate_limiter_priority != Env::IO_TOTAL &&
            rate_limiter_ != nullptr) {
        ...
        read_file_loop_timer.record_phase(0);
      }
      read_file_loop_timer.commit(kReadFileLoop);
