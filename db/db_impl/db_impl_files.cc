...
void DBImpl::PurgeObsoleteFiles(JobContext& state, bool schedule_only) {
  ...
  for (const auto& candidate_file : candidate_files) {
    ...
    if (schedule_only) {
      ...
      if (pirMod::learned_index_enabled.load(std::memory_order_acquire) && type == kTableFile) {
        // printf("Deleting obsolete file: %s\n", fname.c_str());
        bool model_deleted = pirMod::file_data->DeleteModel(number);
        if (!model_deleted) {
          printf("[JOB %d] PIR Model Delete Failed for file number %lu\n",
                          state.job_id, number);
        }
      }
    }
