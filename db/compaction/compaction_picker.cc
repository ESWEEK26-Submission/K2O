...
bool CompactionPicker::SetupOtherInputs(
  ...
  if (!output_level_inputs->empty()) {
    ...
    if (only_expand_towards_right) {
      ...
      // if (input_level==0&&output_level==1) {
      //   std::string s1, s2;
      //   AppendEscapedStringTo(&s1, all_start.user_key());
      //   AppendEscapedStringTo(&s2, all_limit.user_key());

      //   printf("all_start user_key(hex): %s\n", s1.c_str());
      //   printf("all_limit user_key(hex): %s\n", s2.c_str());
      ...
      vstorage->GetOverlappingInputs(input_level, &all_start, &all_limit,
                                     ...
                                     nullptr);
      // if (input_level==0&&output_level==1) {
      //   printf("expanded_inputs file count: %lu\n", expanded_inputs.files.size());
      // }
    }
