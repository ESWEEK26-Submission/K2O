...
std::mutex& CompactionSstBuildTraceMutex() {
  static std::mutex mu;
  return mu;
}

std::unordered_map<uint64_t, uint64_t>& CompactionSstBuildTraceStarts() {
  static std::unordered_map<uint64_t, uint64_t> starts;
  return starts;
}

void StartCompactionSstBuildTrace(uint64_t file_number) {
  std::lock_guard<std::mutex> lock(CompactionSstBuildTraceMutex());
  CompactionSstBuildTraceStarts()[file_number] = rdtscp();
}

void FinishCompactionSstBuildTrace(uint64_t file_number) {
  uint64_t start = 0;
  {
    std::lock_guard<std::mutex> lock(CompactionSstBuildTraceMutex());
    auto& starts = CompactionSstBuildTraceStarts();
    auto it = starts.find(file_number);
    if (it == starts.end()) {
      return;
    }
    start = it->second;
  ...
  LocalOp op;
  op.phase_cycles[0] = rdtscp() - start;
  op.max_phase_idx = 1;
  op.commit(kCompactionSSTBuildTime);
}
...
void CompactionJob::ProcessKeyValueCompaction(SubcompactionState* sub_compact) {
  ...
  input->SeekToFirst();
  if (!input->status().ok()) {
    fprintf(stderr, "input status not ok\n");
  }
  ...
  // printf("[Compaction] Reason: %d\n", static_cast<int>(sub_compact->compaction->compaction_reason()));
  // auto input_files = sub_compact->compaction->get_inputs();
  // for (auto& level_files : *input_files) {
  //   printf("[Compaction] Input files Level: %d, stats:\n", level_files.level);
  //   for (const auto& input_file : level_files.files) {
  //     printf("  Input file number: %lu, size: %lu\n",
  //             input_file->fd.GetNumber(), input_file->fd.GetFileSize());
  //   }
  // }
  op_timer.begin();
  status = sub_compact->CloseCompactionFiles(status, open_file_func,
                                             close_file_func);
  op_timer.record_phase();
  op_timer.commit(kCompactionTime);
  // auto output_files = sub_compact->GetOutputs();
  // printf("[Compaction] Output file stats:\n");
  // for (const auto& output : output_files) {
  //   printf("  Output file number: %lu, size: %lu\n",
  //           output.meta.fd.GetNumber(), output.meta.fd.GetFileSize());
  // }
  // printf("  Input Level: %d, Output Level: %d\n",
...
Status CompactionJob::FinishCompactionOutputFile(
  ...
  if (s.ok() && (current_entries > 0 || tp.num_range_deletions > 0)) {
    ...
    FinishCompactionSstBuildTrace(meta->fd.GetNumber());
  }
  std::string fname;
...
Status CompactionJob::OpenCompactionOutputFile(SubcompactionState* sub_compact,
  ...
  StartCompactionSstBuildTrace(file_number);
