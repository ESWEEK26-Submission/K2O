    ...
    *status = table_cache_->Get(
    ...
    switch (get_context.State()) {
      case GetContext::kNotFound:
        op_timer.commit(kTableGetNotFound);
      ...
      case GetContext::kFound:
        op_timer.commit(kTableGetFound);
...
VersionSet::~VersionSet() {
  ...
  // VersionSet
  if (pirMod::learned_index_enabled.load(std::memory_order_acquire)) {
    column_family_set_->GetDefault()->current()->WriteLevelModel();
  }
...
void Version::WriteLevelModel() {
  Status s;
  for (int i = 0; i < cfd_->NumberLevels(); ++i) {
    for (FileMetaData* file_meta : cfd_->current()->storage_info_.LevelFiles(i)) {
      auto li = pirMod::file_data->GetModel(file_meta->fd.GetNumber());
      if (li.valid()) {
        s = li->WriteModel();
      } else {
        s = Status::InvalidArgument("Write Level model is not valid");
      }
      if (!s.ok()) {
        ROCKS_LOG_ERROR(cfd_->ioptions()->info_log,
                        "Error writing level model for file %lu: %s",
                        file_meta->fd.GetNumber(), s.ToString().c_str());
      }
    }
  }
  ...
  for (int i = 0; i < cfd_->NumberLevels(); ++i) {
    for (FileMetaData* file_meta : cfd_->current()->storage_info_.LevelFiles(i)) {
      ...
      if (li.valid()) {
        ...
        s = Status::InvalidArgument("Read Level model is not valid");
      }
      if (!s.ok()) {
        ROCKS_LOG_ERROR(cfd_->ioptions()->info_log,
                        "Error reading level model for file %lu: %s",
                        file_meta->fd.GetNumber(), s.ToString().c_str());
      }
    }
  }
