...
Status BuildTable(
  ...
  if (iter->Valid() || !range_del_agg->IsEmpty()) {
    LocalOp sst_build_timer;
    sst_build_timer.begin();
    ...
    for (; c_iter.Valid(); c_iter.Next()) {
      ...
      LocalOp add_key_timer;
      add_key_timer.begin();
      builder->Add(key_after_flush, value);
      add_key_timer.record_phase();
      add_key_timer.commit(kFlushAddKey);
    ...
    if (!s.ok() || empty) {
      ...
      // printf("[Flush] BuildTable: Calling Finish on builder for file number %lu\n",
      //        meta->fd.GetNumber());
      s = builder->Finish();
    }
    if (io_status->ok()) {
    ...
    *io_status =
    ...
    sst_build_timer.record_phase();
    sst_build_timer.commit(kSSTBuildTime);
    op_timer.record_phase();
    op_timer.commit(kFlushBuildTableTime);
