...
class CoreWorkload {
 public:
  ...
  /// Options are "uniform", "zipfian", "latest" and "file".
  ///
  static const std::string REQUEST_DISTRIBUTION_PROPERTY;
  static const std::string REQUEST_DISTRIBUTION_DEFAULT;

  ///
  /// The name of the property for the input file path when using "file" distribution.
  ///
  ...
  CoreWorkload()
      : field_count_(0),
        ...
        record_count_(0),
        file_key_generator_(nullptr),
        use_file_keys_(false) {}
  ...
  // For file-based key generation
  class FileGenerator *file_key_generator_;
  bool use_file_keys_;
};
