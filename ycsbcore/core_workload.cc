...
const string CoreWorkload::INPUT_FILE_PROPERTY = "inputfile";
...
void CoreWorkload::Init(const utils::Properties &p) {
  ...
  } else if (request_dist == "file") {
    // Load keys from file
    std::string input_file = p.GetProperty(INPUT_FILE_PROPERTY, INPUT_FILE_DEFAULT);
    if (input_file.empty()) {
      throw utils::Exception("inputfile property must be set when using 'file' distribution");
    }
    
    std::cout << "Using file-based key distribution from: " << input_file << std::endl;
    file_key_generator_ = new FileGenerator(input_file);
    use_file_keys_ = true;
    
    // Update record_count based on loaded keys
    size_t loaded_count = file_key_generator_->GetKeyCount();
    if (loaded_count > 0) {
      record_count_ = loaded_count;
      std::cout << "Updated record_count to " << record_count_ << " based on file" << std::endl;
    }
    
    // For transaction keys, we use uniform distribution over the loaded keys
    key_chooser_ = new UniformGenerator(0, record_count_ - 1);
...
std::string CoreWorkload::BuildKeyName() {
  if (use_file_keys_ && file_key_generator_) {
    // For load phase with file keys, get next key from file
    uint64_t file_key = file_key_generator_->Next();
    return std::to_string(file_key);
  }
...
std::string CoreWorkload::BuildKeyName(uint64_t key_num) {
  // If using file-based keys, get the actual key from file
  if (use_file_keys_ && file_key_generator_) {
    uint64_t file_key = file_key_generator_->GetKey(key_num);
    return std::to_string(file_key);
  }
  
  // Original logic for generated keys
  if (!ordered_inserts_) {
