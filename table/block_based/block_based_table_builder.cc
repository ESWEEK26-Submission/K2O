...
struct BlockBasedTableBuilder::Rep {
  ...
  pirMod::LearnedIndexData* learned_index_data;
  ...
  Rep(const BlockBasedTableOptions& table_opt, const TableBuilderOptions& tbo,
      ...
      : ioptions(tbo.ioptions),
        ...
        status_ok(true),
        io_status_ok(true) {
    if (table_opt.use_learned_index) {
      assert(table_opt.block_restart_interval == 1);
      learned_index_data = pirMod::file_data->NewModel(tbo.cur_file_num);
      data_block.EnablePIR(learned_index_data->NewPIRHandle());
      // fprintf(stderr, "BlockBasedTableBuilder::Rep learned_index_data=%p file_num=%lu\n",
      //        learned_index_data, tbo.cur_file_num);
      pir_enabled = true;
    }
...
void BlockBasedTableBuilder::Add(const Slice& key, const Slice& value) {
  ...
  if (IsValueType(value_type)) {
    ...
    if (should_flush) {
      ...
      if (ok() && r->state == Rep::State::kUnbuffered) {
        if (r->IsParallelCompressionEnabled()) {
          ...
          if (r->pir_enabled) { 
            r->data_block.ResetPIRState(r->pending_handle, r->learned_index_data->NewPIRHandle());
          }
      ...
      flush_timer.record_phase();
      flush_timer.commit(kTableBuilderFlush);
    }
    ...
    add_timer.record_phase();
    add_timer.commit(kTableBuilderAdd);
...
void BlockBasedTableBuilder::WriteBlock(BlockBuilder* block,
  ...
  LocalOp write_block_timer;
  write_block_timer.begin();
  block->Finish();
  write_block_timer.record_phase();
  ...
  write_block_timer.record_phase();
  write_block_timer.commit(kTableBuilderWriteBlock);
}
...
void BlockBasedTableBuilder::WriteBlock(const Slice& uncompressed_block_data,
  ...
  write_block_timer.record_phase();
  write_block_timer.commit(kTableBuilderWriteBlock2);
}
...
void BlockBasedTableBuilder::WriteMaybeCompressedBlock(
  ...
  {
    // printf("写入数据块，偏移量=%lu, 大小=%lu, 压缩类型=%d\n", offset,
    //        block_contents.size(), static_cast<int>(comp_type));
  ...
  op_timer.record_phase();
  op_timer.commit(kTableBuilderWriteBlock3);
}
...
Status BlockBasedTableBuilder::Finish() {
  // printf("BlockBasedTableBuilder::Finish called\n");
  Rep* r = rep_;
  ...
  if (r->IsParallelCompressionEnabled()) {
    ...
    if (ok() && !empty_data_block) {
      if (r->pir_enabled) { 
        ...
        r->data_block.ResetPIRState(r->pending_handle, nullptr);
        r->learned_index_data->TrainRootPIRHandle();
        ...
        pirMod::file_data->FinishModel(r->learned_index_data);
