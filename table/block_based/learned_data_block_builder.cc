...
//
// LearnedDataBlockBuilder generates blocks where keys are prefix-compressed:
//
...
#include "table/block_based/learned_data_block_builder.h"
...
LearnedDataBlockBuilder::LearnedDataBlockBuilder(
...
void LearnedDataBlockBuilder::Reset() {
  buffer_.clear();
...
void LearnedDataBlockBuilder::SwapAndReset(std::string& buffer) {
  std::swap(buffer_, buffer);
  Reset();
}

size_t LearnedDataBlockBuilder::EstimateSizeAfterKV(const Slice& key,
...
Slice LearnedDataBlockBuilder::Finish() {
  printf("LearnedDataBlockBuilder::Finish called\n");
...
void LearnedDataBlockBuilder::Add(const Slice& key, const Slice& value,
...
void LearnedDataBlockBuilder::AddWithLastKey(const Slice& key, const Slice& value,
  ...
  // clever inside LearnedDataBlockBuilder. On this hot code path, we want to avoid
...
inline void LearnedDataBlockBuilder::AddWithLastKeyImpl(const Slice& key,
                                             const Slice& value,
...
const Slice LearnedDataBlockBuilder::MaybeStripTimestampFromKey(std::string* key_buf,
                                                     const Slice& key) {
