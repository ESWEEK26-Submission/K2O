...
class LearnedDataBlockBuilder {
 public:
  LearnedDataBlockBuilder(const LearnedDataBlockBuilder&) = delete;
  void operator=(const LearnedDataBlockBuilder&) = delete;

  explicit LearnedDataBlockBuilder(int block_restart_interval,
  ...
  // Reset the contents as if the LearnedDataBlockBuilder was just constructed.
  void Reset();

  // Swap the contents in LearnedDataBlockBuilder with buffer, then reset the LearnedDataBlockBuilder.
 ...
 private:
  ...
  // before calling `LearnedDataBlockBuilder::Add`.
