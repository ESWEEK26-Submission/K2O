...
Slice BlockBuilder::Finish() {
  ...
  finished_ = true;
  op_timer.record_phase();

  if (pir_enabled_) {
    assert(pir_has_first_);
    // // 计算斜率和截距
    // pir_handle_->intercept =
    //     -pir_handle_->slope * static_cast<double>(pir_handle_->first_key);
    // 使用first key处理后，线性函数无需截距

    uint32_t trained_array_num = static_cast<uint32_t>(ceil(pir_handle_->slope * pir_pending_kvs_.back().key_u64)) + 1;
    pir_handle_->trained_array.assign(trained_array_num, UINT32_MAX);

    for (auto& kv : pir_pending_kvs_) {
      uint32_t idx = static_cast<uint32_t>(ceil(pir_handle_->slope * kv.key_u64));
      pir_handle_->trained_array[idx] = kv.offset;
    }

    // // debug
    // static uint64_t pir_block_count = 0;
    // static uint64_t pir_total_gap_bytes = 0;
    // static std::mutex pir_mtx;
    // {
    //   std::lock_guard<std::mutex> lock(pir_mtx);
    //   pir_block_count++;
    //   pir_total_gap_bytes += (trained_array_num - pir_pending_kvs_.size()) * sizeof(uint32_t);
    //   double avg_gap_bytes = pir_total_gap_bytes * 1.0 / pir_block_count;
    //   if (pir_block_count % 100000 == 0) {
    //     printf("[PIR][BlockBuilder::Finish] kv_cnt=%zu block_size=%lu "
    //     "slope=%.8f "
    //     "pir_total_size=%lu\n",
    //     pir_pending_kvs_.size(), buffer_.size(),
    //     pir_handle_->slope,
    //     pir_handle_->MemoryUsage());

    //     printf("[PIR Total] pir_block_count=%lu avg_gap_bytes=%.2f\n",
    //       pir_block_count, avg_gap_bytes);
    //   }
    // }
    // op_timer.record_phase();
    // // debug end
  }
  op_timer.commit(kBlockBuilderFinish);
...
inline void BlockBuilder::AddWithLastKeyImpl(const Slice& key,
  ...
  if (pir_enabled_) {
    // ======== PIR training (block-local) ========
    uint64_t key_u64 = pirMod::SliceToInteger(key);
    uint32_t cur_offset = static_cast<uint32_t>(buffer_size);

    if (!pir_has_first_) {  // TODO: Only 1 Key Data 
      // ---- first KV in this data block ----
      pir_handle_->first_key = key_u64;
      pir_handle_->last_key = key_u64;
      pir_has_first_ = true;
      // printf("[PIR][BlockBuilder] first KV: key=%lu offset=0\n",
      //         pir_first_key_);
    } else {
      // ---- subsequent KV ----
      assert(key_u64 > pir_handle_->last_key);  // block keys must be strictly increasing

      uint64_t dx = key_u64 - pir_handle_->last_key;
      uint32_t dy = 1 + 1;  // 预测位置存储offset时，相当于存储长度固定为uint32_t的KV对

      // dx must be > 0, dy >= 0
      double slope = static_cast<double>(dy) / static_cast<double>(dx);

      if (slope > pir_handle_->slope) {
        pir_handle_->slope = slope;
      }

      pir_handle_->last_key = key_u64;
    }

    pir_pending_kvs_.push_back({
      key_u64 - pir_handle_->first_key,
      cur_offset
    });
  }
  op_timer.record_phase();
  ...
  op_timer.record_phase();
  op_timer.commit(kAddToBlockBuilder);
}
