...
BlockBasedTable::MaybeReadBlockAndLoadToCache(
  ...
  op_timer.record_phase(7);
  if (is_cache_hit) {
    op_timer.commit(kReadBlockCacheHit);
  } else {
    op_timer.commit(kReadBlockCacheMiss);
    // op_timer.dump();
  }
...
Status BlockBasedTable::LIGet(const ReadOptions& read_options, const Slice& key,
                            GetContext* get_context,
                            const SliceTransform* prefix_extractor,
                            bool skip_filters) {
  // Similar to Bloom filter !may_match
  // If timestamp is beyond the range of the table, skip
  // fprintf(stderr, "BlockBasedTable::LIGet called\n");
  ...
  if (may_match) {
    LocalOp index_op_timer;
    index_op_timer.begin();

    uint64_t file_number = rep_->sst_number_for_tracing();
    // printf("LIGet: SST file number: %lu\n", file_number);
    auto sst_model = pirMod::file_data->GetModel(file_number);
    index_op_timer.record_phase();
    sst_model->SeekToFirst(key);
    index_op_timer.record_phase();
    // static std::mutex li_get_mutex;
    // static int li_get_counter = 0;
    // static int li_loop_counter = 0;
    // static int li_get_0_loop_counter = 0;
    // static int li_get_1_loop_counter = 0;
    // static int li_get_2_loop_counter = 0;
    // static int li_get_gt2_loop_counter = 0;
    // static int li_get_hit_counter = 0;
    // static int li_get_miss_counter = 0;

    // int loop_count = 0;
    // bool hit = false;

    bool matched = false;  // if such user key matched a key in SST
    for (; sst_model->Valid(); sst_model->Next()) {
      // loop_count++;
      pirMod::PIRHandle* pir_handle = sst_model->pir_handle();
      uint64_t pred_offset = static_cast<uint64_t>(sst_model->pred_offset());
      if (pred_offset == UINT32_MAX) {
        // fprintf(stderr, "[LIGet] pred_offset == UINT32_MAX, continue\n");
        if (pirMod::SliceToInteger(key) < pir_handle->last_key) {
          break;
        }
        continue;
      }
      uint64_t block_offset = sst_model->block_offset();
      RandomAccessFileReader* file = rep_->file.get();
      constexpr size_t KV_MAX = 1060; // KV对大小上限
      char scratch[KV_MAX];

      Slice result;
      IOOptions io_opts;
      // fprintf(stderr, "[LIGet] Reading at block offset: %lu, pred_offset: %lu\n",
      //        block_offset, pred_offset);
      Status f_status = file->Read(io_opts, block_offset + pred_offset, KV_MAX, &result, scratch, nullptr);
      if (!f_status.ok()) {
        s = f_status;
        break;
      }
      uint32_t shared, unshared, value_length;
      const char* p = result.data();
      p = GetVarint32Ptr(p, result.data() + result.size(), &shared);
      p = GetVarint32Ptr(p, result.data() + result.size(), &unshared);
      p = GetVarint32Ptr(p, result.data() + result.size(), &value_length);
      // printf("[LIGet] key sizes: shared: %u, unshared: %u, value_length: %u\n",
      //        shared, unshared, value_length);
      assert(p != nullptr && shared == 0); // shared must be zero
      Slice entry_key(p, unshared);
      // printf("------------------------------------------------------\n");
      // printf("[LIGet] entry  key: ");
      // for (size_t i = 0; i < 8; i++) {
      //   printf("%02x ", static_cast<unsigned char>(entry_key[i]));
      // }
      // printf("\n");
      // printf("[LIGet] lookup key: ");
      // for (size_t i = 0; i < 8; i++) {
      //   printf("%02x ", static_cast<unsigned char>(key[i]));
      // }
      // printf("\n");
      if (rep_->internal_comparator.user_comparator()->Compare(ExtractUserKey(entry_key), ExtractUserKey(key)) != 0) {
        if (pirMod::SliceToInteger(key) < pir_handle->last_key) {
          break;
        }
        continue;
      ...
      Slice value = Slice(p + unshared, value_length);
      ...
      Status pik_status = ParseInternalKey(
          entry_key, &parsed_key, false /* log_err_key */);  // TODO
      if (!pik_status.ok()) {
        s = pik_status;
      }

      if (!get_context->SaveValue(
              parsed_key, value, &matched,
              nullptr)) {
        if (get_context->State() == GetContext::GetState::kFound) {
          does_referenced_key_exist = true;
          referenced_data_size = entry_key.size() + value.size();
        }
      }
      if (block_cache_tracer_ && block_cache_tracer_->is_tracing_enabled()) {
        // Avoid making copy of block_key, cf_name, and referenced_key when
        // constructing the access record.
        Slice referenced_key = entry_key;
    ...
    index_op_timer.record_phase();
    index_op_timer.commit(kLIIndexBlockTime);
    ...
    // {
    //   std::lock_guard<std::mutex> lock(li_get_mutex);
    //   if (loop_count == 0) {
    //     li_get_0_loop_counter++;
    //   } else if (loop_count == 1) {
    //     li_get_1_loop_counter++;
    //   } else if (loop_count == 2) {
    //     li_get_2_loop_counter++;
    //   } else {
    //     li_get_gt2_loop_counter++;
    //   }
    //   li_loop_counter += loop_count;
    //   li_get_counter++;
    //   if (hit) {
    //     li_get_hit_counter++;
    //   } else {
    //     li_get_miss_counter++;
    //   }
    //   if (li_get_counter % 10000 == 0) {
    //     fprintf(
    //         stderr,
    //         "[LIGet] total: %d, hit: %d, miss: %d, 0-loop: %d, 1-loop: %d, 2-loop: %d, >2-loop: %d, avg_loop: %.2f\n",
    //         li_get_counter, li_get_hit_counter, li_get_miss_counter,
    //         li_get_0_loop_counter, li_get_1_loop_counter,
    //         li_get_2_loop_counter, li_get_gt2_loop_counter,
    //         li_loop_counter / static_cast<double>(li_get_counter));
    //   }
    // }
  }
...
Status BlockBasedTable::Get(const ReadOptions& read_options, const Slice& key,
                            ...
                            const SliceTransform* prefix_extractor,
                            bool skip_filters) {
  if (rep_->use_learned_index) {
    return LIGet(read_options, key, get_context, prefix_extractor, skip_filters);
  }
  // Similar to Bloom filter !may_match
  // If timestamp is beyond the range of the table, skip
  if (!TimestampMayMatch(read_options)) {
  ...
  if (may_match) {
    ...
    for (iiter->Seek(key); iiter->Valid() && !done; iiter->Next()) {
      ...
      uint64_t referenced_data_size = 0;
      Status tmp_status;
      index_op_timer.record_phase(2);
      LocalOp data_op_timer;
      data_op_timer.begin();
      ...
      data_op_timer.record_phase(2);
      data_op_timer.commit(kDataBlockTime);
    ...
    index_op_timer.commit(kIndexBlockTime);
    if (matched && filter != nullptr) {
      if (rep_->whole_key_filtering) {
        RecordTick(rep_->ioptions.stats, BLOOM_FILTER_FULL_TRUE_POSITIVE);
      } else {
...
Status BlockBasedTable::CreateIndexReader(
  ...
  switch (rep_->index_type) {
    ...
    case BlockBasedTableOptions::kLearnedIndexSearch: {
      return LearnedIndexReader::Create(this, ro, prefetch_buffer, use_cache,
                                    prefetch, pin, lookup_context,
                                    index_reader);
    }
    default: {
