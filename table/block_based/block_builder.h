...
class BlockBuilder {
 public:
  ...
  inline void EnablePIR(pirMod::PIRHandle* pir_handle) {
    pir_enabled_ = true;
    pir_handle_ = pir_handle;
  }

  inline bool PIREnabled() {
    return pir_enabled_;
  }

  inline void ResetPIRState(BlockHandle& handle, pirMod::PIRHandle* pir_handle) {
    pir_has_first_ = false;
    pir_pending_kvs_.clear();
    pir_handle_->block_offset = handle.offset();
    pir_handle_ = pir_handle;
  }
 ...
 private:
  ...
  // learned index (PIR)
  pirMod::PIRHandle* pir_handle_;

  bool pir_has_first_ = false;

  struct PIRPendingKV {
    uint64_t key_u64;
    uint32_t offset;
  };
  std::vector<PIRPendingKV> pir_pending_kvs_;
};
