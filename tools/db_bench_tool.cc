...
DEFINE_string(latency_stats_dir, "",
              "Directory path to save read/write latency statistics. "
              "If empty, stats will not be saved to files.");
...
DEFINE_bool(use_hash_search, false,
            ...
            "This is valid if only we use BlockTable");
DEFINE_bool(use_learned_index, false,
            "if use learned index instead of binary search. "
            "This is valid if only we use BlockTable");
static const char kFixedLearnedIndexType[] = "allinmemory";
static const uint64_t kFixedLearnedIndexCacheSizeMb = 1024;
static const bool kFixedUseSecondaryLearnedIndex = true;
...
class Stats {
 ...
 public:
  ...
  void Report(const Slice& name) {
    ...
    // Save latency statistics to file if directory is specified
    SaveLatencyStats(name);
    
    fflush(stdout);
  }

  void SaveLatencyStats(const Slice& name) {
    if (FLAGS_latency_stats_dir.empty()) {
      return;
    }
    
    // Create directory if it doesn't exist
    std::string dir = FLAGS_latency_stats_dir;
    Status s = FLAGS_env->CreateDirIfMissing(dir);
    if (!s.ok()) {
      fprintf(stderr, "Failed to create latency stats directory: %s\n", 
              s.ToString().c_str());
      return;
    }

    // Save histogram data for each operation type
    for (auto it = hist_.begin(); it != hist_.end(); ++it) {
      std::string op_name = OperationTypeString[it->first];
      std::string filename = dir + "/" + name.ToString() + "_" + op_name + "_latency.txt";
      
      // Open file for writing
      std::unique_ptr<WritableFile> file;
      EnvOptions env_options;
      s = FLAGS_env->NewWritableFile(filename, &file, env_options);
      if (!s.ok()) {
        fprintf(stderr, "Failed to create latency stats file %s: %s\n",
                filename.c_str(), s.ToString().c_str());
        continue;
      }

      // Write header
      std::string header = "# Latency statistics for " + name.ToString() + 
                          " - " + op_name + "\n";
      file->Append(header);
      
      // Write histogram data
      std::string hist_str = it->second->ToString();
      file->Append(hist_str);
      file->Append("\n");
      ...
      fprintf(stdout, "Saved %s latency stats to: %s\n", 
              op_name.c_str(), filename.c_str());
    }
  }
};
    ...
    while (std::getline(benchmark_stream, name, ',')) {
      ...
      if (method != nullptr) {
        ...
        std::string breakdown_json =
            name + (FLAGS_use_learned_index ? "_li" : "_no_li") +
            std::string("_stats.json");
        if (!FLAGS_latency_stats_dir.empty()) {
          std::string dir = FLAGS_latency_stats_dir;
          if (!dir.empty() && dir.back() == '/') {
            dir.pop_back();
          }
          breakdown_json = dir + "/" + breakdown_json;
        }
        dump_json(breakdown_json.c_str());
        if (!FLAGS_use_learned_index) {
          assert(pirMod::file_data == nullptr);
        }
 ...
 private:
  ...
  void InitializeOptionsFromFlags(Options* opts) {
    ...
    if (FLAGS_use_plain_table) {
      ...
      } else if (FLAGS_use_learned_index) {
        block_based_options.index_type = BlockBasedTableOptions::kLearnedIndexSearch;
      } 
      else {
      ...
      if (FLAGS_use_learned_index) {
        block_based_options.use_learned_index = true;
        block_based_options.block_restart_interval = 1;
        pirMod::learned_index_enabled.store(true, std::memory_order_release);
        if (std::string(kFixedLearnedIndexType) == "cache") {
          pirMod::li_type = pirMod::FileLearnedIndexData::Mode::kCache;
        } else if (std::string(kFixedLearnedIndexType) == "allinmemory") {
          pirMod::li_type = pirMod::FileLearnedIndexData::Mode::kAllInMemory;
        } else {
          pirMod::li_type = pirMod::FileLearnedIndexData::Mode::kNoCache;
        }
        pirMod::li_cache_size =
            static_cast<size_t>(kFixedLearnedIndexCacheSizeMb * 1024 * 1024);
        pirMod::use_secondary_learned_index = kFixedUseSecondaryLearnedIndex;
        printf("[PIR] Learned index is enabled, li_type:%s\n",
               kFixedLearnedIndexType);
      }
    ...
    if (run) {
      ...
      while (!duration.Done(1)) {
        ...
        switch (workload->NextOp()) {
          case ycsbc::READ: {
            op_status = db->Get(r_op, key, &data);
            if (op_status.ok()) {
              found_count++;
          ...
          case ycsbc::SCAN: {
            Iterator* db_iter = db->NewIterator(rocksdb::ReadOptions());
            db_iter->Seek(key);
