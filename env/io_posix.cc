...
IOStatus PosixRandomAccessFile::Read(uint64_t offset, size_t n,
  ...
  while (left > 0) {
    LocalOp pread_timer;
    pread_timer.begin();
    // static size_t pread_counter = 0;
    // static size_t pread_bytes = 0;
    // static size_t pread_max_bytes = 0;
    // static std::mutex pread_mutex;
    // {
    //   std::lock_guard<std::mutex> lock(pread_mutex);
    //   pread_counter++;
    //   pread_bytes += n;
    //   pread_max_bytes = std::max(pread_max_bytes, n);
    //   if (pread_counter % 10000 == 0) {
    //     printf("[pread] count: %zu, avg_bytes: %.2f, max_bytes: %zu\n", pread_counter, pread_bytes / static_cast<double>(pread_counter), pread_max_bytes);
    //   }
    // }
    r = pread(fd_, ptr, left, static_cast<off_t>(offset));
    pread_timer.record_phase();
    pread_timer.commit(kReadRandomAccessFile);
    if (r <= 0) {
      if (r == -1 && errno == EINTR) {
        continue;
      }
